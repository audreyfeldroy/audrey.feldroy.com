# AUTOGENERATED! DO NOT EDIT! File to edit: articles/2025-02-07-This-Site-Is-Now-Powered-by-This-Notebook-Part-6.ipynb.
...
def get_nb_paths():
    root = Path() if IN_NOTEBOOK else Path("articles/")
...
def mk_nbcard_from_nb_path(nb_path):
    ...
    return NBCard(
    *get_title_and_desc(nb_path), href=f"/articles/{nb_path.name[:-6]}", date=date
    )
...
def notebook(name: str):
    fname = f"{name}.ipynb" if IN_NOTEBOOK else f"articles/{name}.ipynb"
    fpath = Path(fname)
    nb = read_nb(fpath)
    title = nb.cells[0].source.lstrip("# ")
    date = get_date_from_iso8601_prefix(fname.lstrip("articles/"))
    desc = nb.cells[1].source
